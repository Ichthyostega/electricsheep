...
void mpeg2_idct_init (uint32_t mm_accel)
{
    ...
    if (mm_accel & MM_ACCEL_X86_MMXEXT) {
      if (!quiet) fprintf (stderr, "Using MMXEXT for IDCT transform\n");
	mpeg2_idct_copy = mpeg2_idct_copy_mmxext;
	mpeg2_idct_add = mpeg2_idct_add_mmxext;
	mpeg2_idct_mmx_init ();
    } else if (mm_accel & MM_ACCEL_X86_MMX) {
      if (!quiet) fprintf (stderr, "Using MMX for IDCT transform\n");
    ...
    if (mm_accel & MM_ACCEL_PPC_ALTIVEC) {
      if (!quiet) fprintf (stderr, "Using altivec for IDCT transform\n");
	...
	if (env_var == NULL) {
	  if (!quiet) fprintf (stderr, "Using mlib for IDCT transform\n");
	    mpeg2_idct_add = mpeg2_idct_add_mlib;
	} else {
	  if (!quiet) fprintf (stderr, "Using non-IEEE mlib for IDCT transform\n");
	...
	if (!quiet) fprintf (stderr, "No accelerated IDCT transform found\n");
